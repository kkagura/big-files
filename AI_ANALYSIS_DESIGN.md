...
Markdown 是面向用户阅读的报告，风险等级必须结合磁盘清理候选的上下文显示为中文：`likely_safe` 显示为“低风险候选”、`review` 显示为“需人工确认”、`keep` 显示为“建议保留”、`unknown` 显示为“信息不足”。其中“低风险候选”只表示适合优先核查，不代表文件绝对安全或可以直接删除。JSON 继续保留稳定的英文枚举值，供程序校验和自动化处理。
...
- 内置工具的 JSON Schema 在发送模型请求前必须通过本地 JSON 有效性校验；校验失败时错误应包含工具名称，且不得发送网络请求。
...
- CLI 应在配置加载、扫描开始与完成、长扫描周期统计、每轮模型等待与返回、只读工具调用、最终结果提交和报告写入时输出简短进度。交互式终端等待模型时使用单行旋转 loading，响应或错误后清理动画并显示耗时；非交互输出使用普通静态文本，不写终端控制字符。进度信息不得包含 API Key、完整模型请求正文或大批量路径元数据。
- 每个只读工具调用要求模型提供不超过 200 字的 `decision_summary` 和 `reason`，CLI 分别显示为本轮决策摘要和工具调用理由；最终结果也提供决策摘要。这些字段是面向用户的可审计说明，不是隐藏思维链。输出前必须移除控制字符、折叠换行并限制长度；重复查询判定忽略说明字段，防止仅改写理由绕过保护。
